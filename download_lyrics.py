...
artists: list = ["gigi_dalessio", "nino_dangelo", "pino_daniele"]
...
for artist in artists:
    URL: str = f"https://www.lyricsmania.com/{artist}_lyrics.html"
    soup = soupify(URL)
...
for i, url in tqdm(enumerate(songs_urls), total=len(songs_urls)):
    # if i == THRES:
    #     break
    # To avoid IP ban - setting a five seconds stop after scraping 20 songs
    if i % 20 == 0:
        time.sleep(5)
